...
5. Тот же самый файл отдайте игрокам. Он один на всех: сервер сам расскажет клиенту, что за сборка и откуда её брать.
...
- Она называет мод на сервере, который добавляет записи в реестры игры, но не выдаётся игрокам: без него новому игроку откажет сама игра, ещё до вопроса UDMC.
...
Проект разработан и поддерживается с активным использованием ИИ-ассистентов: они пишут код, тесты и документацию под управлением и с проверкой человека. Каждый выпуск проходит автоматические тесты (123 проверки интерфейса и протокола, тесты Rust и Java) и ручную проверку на настоящем Minecraft-сервере.
