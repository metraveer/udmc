...
Повторный запуск открывает уже работающее окно, в том числе свёрнутое.

Обновление панели сохраняет прежний выбор загрузчика. Агент на сервере обновляется из раздела **«Агенты и игроки»**: на Linux файл подменяется сразу и вступает в силу при следующем запуске, на Windows его заменяет процесс-помощник после выхода сервера; старый рядом не оставляют. Добавление NeoForge не требует переводить существующий Fabric-сервер на другой загрузчик.
...
Публично нужны `GET /health`, `/manifest`, `/files/*`, `/udmc`, `/agents/*` и `/pair` (без ключа по замыслу: отвечает только до привязки, не больше 10 попыток в минуту с одного IP). `/admin/*` требует ключ; `/access/*` используется для приглашений и их проверки. Дополнительно ограничьте оба административных префикса доверенными IP или VPN и разрешите доступ приглашённому компьютеру. Прокси должен сохранять заголовок `x-udmc-signature`, не менять тело `/manifest`, не кэшировать manifest и ответы доступа, принимать загрузки до 512 МиБ. Настройте лимиты частоты и длительности запросов.

Если прокси работает на другой машине или в другом контейнере, `127.0.0.1` не будет адресом агента. Оставьте `"apiHost": "0.0.0.0"` (значение по умолчанию) в `config/udmc-sync.json` и настройте firewall, разрешающий доступ только прокси.

### Локальная сеть без HTTPS

Для доверенной LAN можно указать `192.168.1.50:3077` и явно разрешить HTTP в редакторе адреса перед нажатием **Применить**. Адрес UDMC вводится в «Подключении» один раз и передаётся серверу при привязке; при вводе другого адреса разрешение HTTP сбрасывается. Подпись защищает клиентские обновления от подмены, но HTTP не скрывает токен и файлы. Для интернета этот режим не подходит.

Сервер на том же компьютере — это `http://127.0.0.1:3077`; адрес указывают в редакторе и применяют. Это не публичный адрес для игроков: сервер называет клиентам тот адрес, который панель передала ему при привязке, поэтому для сервера дома укажите внешний адрес, LAN-адрес или адрес VPN, доступный игрокам. UDMC не настраивает роутер, DNS, сертификаты и брандмауэр автоматически. Поля с вопросительным знаком раскрывают краткие объяснения.
...
Разрешены `mods/`, `config/`, `resourcepacks/`, `shaderpacks/`. Максимум файла 512 МиБ; локальный анализ ZIP/JAR ограничен 64 МиБ. Для ручной загрузки NeoForge-JAR и слишком больших JAR нужно выбрать сторону: TOML не описывает надёжно назначение всего файла. Перед публикацией сервер проверяет метаданные своего загрузчика, стороны зависимостей, обязательные зависимости, заявленные несовместимости и дубли ID, включая вложенные моды. Тот же мод, уже лежащий в черновике или на сервере под другим именем, заменяется новой версией — строка файла говорит, что именно она заменит и уберёт. Проверка метаданных не гарантирует отсутствие ошибок внутри модов или сохранность мира.
...
Три источника на одной странице: **Modrinth**, **GitHub Releases** и **CurseForge**. Порядок для Modrinth:
...
2. При необходимости введите название и нажмите **Найти**, смените сортировку или страницу.
3. Выберите мод и версию. По умолчанию выбирается стабильный релиз.
4. Нажмите **Подобрать зависимости**: появятся обязательные моды, их назначение, версии и имена файлов. Ненужную зависимость можно снять галочкой; та, что уже лежит в черновике той же версией, отмечена и не добавляется, а старая версия будет заменена — об этом сказано в её строке.
...
Вход в аккаунт не нужен. Поддерживаются Fabric- и NeoForge-моды для встроенных профилей, до 48 файлов и 256 МиБ за операцию, каждый JAR до 64 МиБ. Назначение берётся из данных автора на Modrinth. Проверяются HTTPS, официальный CDN, размер и SHA-512. Внешние обязательные зависимости, неоднозначные версии, циклы и неизвестное назначение останавливают импорт. Необязательные и уже встроенные зависимости отдельно не скачиваются. Старая версия того же мода в черновике или на сервере заменяется, а не остаётся рядом; что именно будет заменено и убрано с сервера, показано до добавления.

**GitHub Releases** — вкладка рядом: укажите репозиторий, выберите выпуск и JAR из его файлов. Ключ не нужен. **CurseForge** — нужен личный бесплатный ключ API; он вводится в самой вкладке, хранится в Windows Credential Manager и отправляется только CurseForge. Загрузки с CurseForge проверяются по размеру и SHA-1. Зависимости, замена старых версий и подтверждение перед добавлением работают одинаково для всех трёх источников.
...
**Сборка → Вне сборки** перечисляет неуправляемые файлы разрешённых папок. Выберите сторону и нажмите **Под управление**. Сначала опубликуйте добавление: UDMC начнёт учитывать файл. После этого его можно убрать обычным удалением из сборки и новой публикацией.
...
На Windows игра может удерживать JAR. Если файл занят, операция завершится ошибкой; может потребоваться замена при остановленной игре/сервере вручную. Обновление самого агента устроено иначе: на Windows файл подменяет отдельный процесс-помощник уже после выхода сервера, поэтому занятый JAR ему не помеха.
...
После перезапуска сервера откройте **Консоль → Подключение RCON**, нажмите карандаш, укажите домен/IP без протокола, порт и пароль, затем **Применить**. Это сохраняет настройки, но не проверяет соединение. **Проверить** и выполнение команды обновляют индикаторы в консоли и сайдбаре. Соединение открывается на время команды: **Доступен** означает успешную последнюю проверку, не постоянный сокет. Через минуту индикатор просит повторить проверку.

Пароль хранится в Windows Credential Manager отдельно для каждого сервера панели. RCON не шифруется: используйте VPN, SSH-туннель или консоль UDMC по HTTPS.
...
На Windows (или если подмена на месте не удалась) обновление агента применяется после выхода сервера: помощник дожидается завершения процесса и подменяет JAR, повторяя попытку до полуминуты, если Windows ещё держит файл. Wrapper из репозитория перед новым запуском сам вызывает помощника с `--finish`. Если сервер перезапускает supervisor или панель хостинга, дайте помощнику закончить: либо вызовите перед запуском `java -cp udmc-sync/agent-update/helper.jar dev.udmc.sync.update.AgentUpdateHelper --finish udmc-sync/agent-update/task.properties`, как это делает wrapper, либо поставьте задержку перезапуска не меньше 30 секунд. Иначе старый JAR запустится раньше подмены, обновление не применится, и панель по-прежнему покажет прежнюю версию.
