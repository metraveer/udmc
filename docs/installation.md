...
2. Нажмите карандаш рядом с **Версия игры**, выберите загрузчик, затем Minecraft. Доступны Fabric `1.21.1` (Java 21), `26.1.2` и `26.2` (Java 25) с Loader `0.19.3`, а также NeoForge `1.21.1` / `21.1.248` (Java 21). Версия загрузчика берётся из встроенного каталога. Нажмите **Применить**. NeoForge 26.x и Forge пока недоступны.
...
5. В панели откройте **Настройка сервера → Привязка**, укажите адрес UDMC и введите код. Если у сервера включён RCON, кнопка **«Взять код»** заберёт его сама — тогда достаточно пароля RCON, который выдаёт хостинг.
