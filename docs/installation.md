...
На Windows игра может удерживать JAR. Если файл занят, операция завершится ошибкой; может потребоваться замена при остановленной игре/сервере вручную. Обновление самого агента устроено иначе: файл подменяет отдельный процесс-помощник уже после выхода сервера, поэтому занятый JAR ему не помеха.
...
Обновление агента применяется после выхода сервера: помощник дожидается завершения процесса и подменяет JAR, повторяя попытку до полуминуты, если Windows ещё держит файл. Wrapper из репозитория перед новым запуском сам вызывает помощника с `--finish`. Если сервер перезапускает supervisor или панель хостинга, дайте помощнику закончить: либо вызовите перед запуском `java -cp udmc-sync/agent-update/helper.jar dev.udmc.sync.update.AgentUpdateHelper --finish udmc-sync/agent-update/task.properties`, как это делает wrapper, либо поставьте задержку перезапуска не меньше 30 секунд. Иначе старый JAR запустится раньше подмены, обновление не применится, и панель по-прежнему покажет прежнюю версию.
